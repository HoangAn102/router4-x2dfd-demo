#!/usr/bin/env bash
# CLEAN_REBUILD_V2 — canonical isolated runtimes
set -Eeuo pipefail
...
RUNROOT="$ROOT/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP"
...
DATA="/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/eval_shadow"
X2="$BASE/projects/X2DFD"
INT="$BASE/projects/x2dfd_router_integration"

PY_X2="/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/runtime_clean_v2/x2python"
PY_FREQ="/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/runtime_clean_v2/freqpython"
PY_TEX="/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/runtime_clean_v2/texpython"
...
"$PY_X2" - \
 "$DATA" \
 "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/schema_template" \
 "$MANIFEST" \
 "$LABELS" \
 "$LOCK" \
 "$CELEB_JSON" \
...
###############################################################################
# DFDC — INTENTIONALLY EXCLUDED FROM THIS 2-CROSS BENCHMARK
###############################################################################

# Keep metadata path defined only for provenance / legacy positional interfaces.
# It MUST contain zero actual evaluation rows.
dfdc_labels = DATA / "DFDC" / "test" / "labels.csv"

if not dfdc_labels.is_file():
    raise RuntimeError(
        f"2-cross DFDC exclusion sentinel missing: {dfdc_labels}"
    )
...
with dfdc_labels.open(
    ...
    _dfdc_placeholder_rows = list(csv.DictReader(f))

if _dfdc_placeholder_rows:
    raise RuntimeError(
        f"DFDC MUST be excluded, but placeholder has "
        f"{len(_dfdc_placeholder_rows)} rows"
    )

dfdc_video_count = 0

print("[2-CROSS] DFDC intentionally excluded: 0 videos / 0 frames")
...
# FINAL_MATERIALIZE_X2DFD_INPUTS_V2
echo "===== FINAL INPUT MATERIALIZATION ====="
"$PY_X2" -u "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/materialize_x2dfd_inputs_FINAL.py" "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/schema_template/PROVEN_X2DFD_TEMPLATE.json" "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/smoke/work/manifest.csv" "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/smoke" "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/smoke/work/full_inputs.json" "$0"
test -s "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/smoke/work/full_inputs.json" || { echo "full_inputs.json missing"; exit 1; }
echo "✅ FINAL INPUT MATERIALIZATION PASS"
...
CANONICAL_PATCH_CFG="/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/patch_config_CANONICAL.py"

[ -s "$CANONICAL_PATCH_CFG" ] || {
    echo "[ERROR] canonical patch_config missing: $CANONICAL_PATCH_CFG"
    exit 1
}

cp -f "$CANONICAL_PATCH_CFG" "$PATCH_CFG"

"$PY_X2" -m py_compile "$PATCH_CFG"

echo "✅ canonical patch_config installed"
...
###############################################################################
# >>> CANONICAL_STAGE_N_PAIR_CHECK >>>
# >>> CANONICAL_STAGE_N_CACHE_ALIGN >>>
"$PY_X2" "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/align_router_cache_to_config.py" \
 "$SMOKE_CFG_R4" \
 "$SMOKE/work/router_moe_cache.csv" \
 "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/cache_alignment_audit/smoke_runtime_alignment.json"
# <<< CANONICAL_STAGE_N_CACHE_ALIGN <<<

"$PY_X2" "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/validate_config_pair.py" "$SMOKE_CFG_ORIG" "$SMOKE_CFG_R4" "$SMOKE/work" smoke-runtime 8
# <<< CANONICAL_STAGE_N_PAIR_CHECK <<<
...
# X2DFD_CONTINUOUS_SCORE_GATE_V2
...
def inspect_record(rec):
    ...
    if len(real) != 1:
        ...
        return {
            "ok": False,
            "reason":
                "missing_or_ambiguous_real_score",
            "values":
                real,
        }
    ...
    if len(fake) != 1:
        ...
        return {
            "ok": False,
            "reason":
                "missing_or_ambiguous_fake_score",
            "values":
                fake,
        }
    ...
    if not (
        ...
        return {
            "ok": False,
            "reason":
                "score_outside_probability_range",
            "real_score": r,
            "fake_score": f,
        }
...
report = {
    "contract":
        "continuous final MLLM REAL/FAKE probability pair",
    "hard_text_conversion":
        False,
    "expert_score_substitution":
        False,
    "json_files_scanned":
        len(json_files),
    "conversation_result_files_found":
        len(result_files),
...
# >>> EXPLICIT_X2DFD_SMOKE_JSON_BEGIN >>>
SMOKE_2CROSS_JSON="$SMOKE/FULL_2CROSS_INPUT.json"

"$PY_X2" "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/build_exact_2cross_input.py" \
 "$SMOKE_2CROSS_JSON" \
 "$SMOKE/Celeb-DF-v2_smoke.json" \
 "$SMOKE/DFDCP_smoke.json"

[ -s "$SMOKE_2CROSS_JSON" ] || \
    die "Explicit smoke --json was not created"

# <<< EXPLICIT_X2DFD_SMOKE_JSON_END <<<

stage "Q. ORIGINAL X2DFD SMOKE INFERENCE"

# >>> FRESH_INFERENCE_Q >>>
rm -f "$X2/eval/outputs/infer/latest_run.json"
rm -rf "$SNAP/smoke_original"
rm -f "$SMOKE/original_token_score_gate.json"
# <<< FRESH_INFERENCE_Q <<<
...
# >>> FRESH_INFERENCE_R >>>
rm -f "$X2/eval/outputs/infer/latest_run.json"
rm -rf "$SNAP/smoke_router4"
rm -f "$SMOKE/router4_token_score_gate.json"
# <<< FRESH_INFERENCE_R <<<
...
# >>> CANONICAL_FULL_RESUME_SANITIZER >>>
"$PY_X2" "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/sanitize_resume.py" "$WORK" "$CELEB_JSON" "$DFDCP_JSON"
# <<< CANONICAL_FULL_RESUME_SANITIZER <<<
...
CUDA_VISIBLE_DEVICES=0 \
 ...
 "$RUN_FREQ" \
 --out "$WORK" \
 --name PAPER_FINAL_2CROSS_CDF_DFDCP \
 2>&1 \
...
# >>> CANONICAL_STAGE_W_PAIR_CHECK >>>
# >>> CANONICAL_STAGE_W_CACHE_ALIGN >>>
"$PY_X2" "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/align_router_cache_to_config.py" \
 "$FULL_CFG_R4" \
 "$WORK/router_moe_cache.csv" \
 "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/cache_alignment_audit/full_runtime_alignment.json"
# <<< CANONICAL_STAGE_W_CACHE_ALIGN <<<

"$PY_X2" "/home/aiotlab/hoangan/outputs/ROUTER4_FINAL_CLEAN_RETRAIN_20260919/06_crossdata_benchmark/PAPER_FINAL_2CROSS_CDF_DFDCP/validate_config_pair.py" "$FULL_CFG_ORIG" "$FULL_CFG_R4" "$WORK" full-runtime -1
# <<< CANONICAL_STAGE_W_PAIR_CHECK <<<
...
# >>> EXPLICIT_X2DFD_FULL_JSON_BEGIN >>>
FULL_2CROSS_JSON="$WORK/FULL_2CROSS_INPUT.json"
...
[ -s "$FULL_2CROSS_JSON" ] || \
    die "Explicit full --json was not created"

# <<< EXPLICIT_X2DFD_FULL_JSON_END <<<
...
rm -f "$X2/eval/outputs/infer/latest_run.json"
rm -rf "$SNAP/original"
rm -f "$RUNROOT/original_token_score_gate.json"
# <<< FRESH_INFERENCE_X <<<
...
rm -f "$X2/eval/outputs/infer/latest_run.json"
rm -rf "$SNAP/router4"
rm -f "$RUNROOT/router4_token_score_gate.json"
# <<< FRESH_INFERENCE_Y <<<
